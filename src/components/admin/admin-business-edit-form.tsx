...
import { updateBusinessFromAdmin } from "@/components/admin/actions";
...
export function AdminBusinessEditForm({ business }: { business: EditableAdminBusiness }) {
  ...
  async function saveListing(formData: FormData) {
    ...
    const result = await updateBusinessFromAdmin(business.id, formData);
    setIsSaving(false);
    ...
    setMessage({ tone: "success", text: "Listing updated." });
  }

  return (
    <form action={saveListing} className="grid gap-5 border border-[#ded8cc] bg-white p-5">
      <div>
        <p className="text-sm font-semibold uppercase tracking-wide text-[#9c4f35]">Admin edit</p>
        <h2 className="mt-1 text-2xl font-semibold">Edit listing details</h2>
        <p className="mt-2 text-sm leading-6 text-[#6d675d]">
          Save corrections directly to this listing. Public pages update after the save completes.
        </p>
      </div>
      {message ? (
        <p
