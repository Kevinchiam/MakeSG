...
import { updateBusinessMediaFromAdmin } from "@/components/admin/actions";
...
export function AdminBusinessMediaForm({
  businessId,
  portfolio,
}: {
  businessId: string;
  ...
  async function saveMedia(formData: FormData) {
    ...
    const result = await updateBusinessMediaFromAdmin(businessId, formData);
    setIsSaving(false);
    ...
    setNewFiles([]);
    setNewCaptions({});
    setMessage({ tone: "success", text: "Portfolio media updated." });
  }

  return (
    <form action={saveMedia} className="grid gap-5 border border-[#ded8cc] bg-white p-5">
      <div>
        <p className="text-sm font-semibold uppercase tracking-wide text-[#9c4f35]">Admin media edit</p>
        <h2 className="mt-1 text-2xl font-semibold">Edit portfolio media</h2>
        <p className="mt-2 text-sm leading-6 text-[#6d675d]">
          Change captions, remove old uploads, or add new photos and videos. The first image becomes the listing image.
        </p>
      </div>
      {message ? (
        <p
      ...
      {visibleItems.length ? (
        ...
        <p className="border border-dashed border-[#ded8cc] p-4 text-sm text-[#6d675d]">
          No portfolio media will remain unless you add new uploads below.
        </p>
      )}
      <FileUploader
        accept="media"
