...
- Business onboarding can optionally request an AI-assisted Magic fill draft after a business name is typed. The Magic fill affordance is hidden from public visitors when OpenAI is missing or the lightweight availability check fails, including exhausted API credits. Signed-in admins see a small diagnostic note explaining why Magic fill is paused. When available, the draft uses OpenAI web search, fills blank form fields only, attempts to suggest one profile image from the business's own website, and leaves the final editable submission in the existing review flow.
...
- Blank image captions can be described by OpenAI after upload when `OPENAI_API_KEY` is configured. Clearing an existing image caption during edit can also regenerate from the saved image URL. If AI captioning is unavailable, the media is a video, or the contributor wrote a caption, MakeSG uses the existing simple context-aware caption fallback. Admins can run an AI caption check from `/admin` to confirm whether the deployed site can see the key and receive a response from OpenAI without exposing the secret. The diagnostic uses a small PNG served from the site so it tests a normal public image URL rather than inline image data.
...
- `GET /api/ai-caption-test-image`: Serves a tiny public PNG used by the admin AI caption diagnostic.
